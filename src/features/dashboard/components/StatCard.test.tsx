...
describe('StatCard Component', () => {
  ...
  it('displays trend indicator when trend prop is provided', () => {
    render(<StatCard label="Metrics" value={100} trend={{ value: 15, direction: 'up' }} />);
  ...
  it('displays down trend with correct styling', () => {
    const { container } = render(
      <StatCard label="Errors" value={5} trend={{ value: 10, direction: 'down' }} />
    );
    expect(screen.getByText(/↓/)).toBeInTheDocument();
  ...
  it('applies custom color to value', () => {
    const { container } = render(<StatCard label="Custom" value={99} color="text-blue-600" />);
